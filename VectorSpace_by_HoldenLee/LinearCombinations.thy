...
text \<open>A submodule is a set $N\subseteq M$ that is closed under addition and scalar multiplication, and
contains 0.\<close>
\<^cancel>\<open>
locale submodule =
...
text \<open>Note: one can define $M^n$ from this. to-do\<close>
