...
\begin{itemize}
	...
	\item verbose, e.g.\ need to annotate (almost) every $+$ \pause %also: cannot simply write α ∈ L
...
\item $\QQ(\sqrt{3})/\QQ$ is a field extension. %Field extensions like these are interesting when doing Galois theory.
%to-do: explain algebraic
\end{itemize}
\end{frame}
...
\begin{itemize}
	\item missing lemmas, missing definitions\pause %that's how we got here
...
%I have proven this, in fact also used it in the classification of .
%to-do: really interesting?
\end{frame}

\begin{frame}%to-do: show lemma and Int_ring_def the slide before
